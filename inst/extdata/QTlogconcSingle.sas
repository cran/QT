ods trace output;
ods pdf file="QTlogconcAnalysis.pdf";*pathname for the output;

libname original xport "data.xpt";
libname derived "";
options nofmterr;
proc copy in=original out=derived;
run;
data old1; 
	set derived.sas;
	DQTCOBS=qtc_cfb;
...
CLASS ID;
MODEL DQTCOBS=CONC/ SOLUTION CL ALPHA=0.1 ALPHAP=0.1 DDFM= KENWARDROGER
outpm=pred1
...
PROC MIXED DATA =OLD3 METHOD=ML;
CLASS ID;
MODEL DQTCOBS=CONC/ SOLUTION CL ALPHA=0.1 ALPHAP=0.1 NOINT DDFM= KENWARDROGER
